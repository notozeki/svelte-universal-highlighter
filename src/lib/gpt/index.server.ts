...
const DEFAULT_OPENAI_API_KEY = env.VITE_OPENAI_KEY;

async function textCompletion(apiKey: string, prompt: string) {
	const configuration = new Configuration({ apiKey: apiKey || DEFAULT_OPENAI_API_KEY });
...
async function completeColor(
	...
	return textCompletion(apiKey, prompt).then((res) => {
		...
		return Object.fromEntries(
			colorsString.split(',').map((color, idx) => [tokenTypes[idx], color])
		);
	});
}
